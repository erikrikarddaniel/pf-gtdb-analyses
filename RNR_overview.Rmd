...
```{r constants}
N_TOP_PHYLA = 20        # Number of top phyla to present in tables and figures
```
...
major_phyla <- taxa %>% 
  ...
  group_by(tdomain) %>% top_n(N_TOP_PHYLA, n_genomes) %>% ungroup() %>%
...
major_phyla <- major_phyla %>% 
  ...
  group_by(tdomain, major_phylum) %>% mutate(n_genomes = sum(n_genomes)) %>% ungroup() %>%
...
See `r figr('genomes-wo-rnr-alpha', T, type = 'Table')` for a list of families containing genomes without RNR alpha

subunits.

```{r missing-rnr-completeness, fig.height = 3, fig.cap = '**Number of species representative GTDB genomes containing no identifiable RNR alpha subunit as a function of genome completeness.**'}
taxa %>% transmute(
  genome_accno, 
  tdomain, 
...
genomes_wo_rnr_alpha %>%
  ...
  group_by(tdomain, major_phylum, n_genomes) %>% summarise(n = sum(n), .groups = 'drop') %>%
...
```{r genomes-wo-rnr-alpha, fig.cap = "**GTDB families with genomes not encoding any identified RNR alpha subunit.**", cache = FALSE}
genomes_wo_rnr_alpha %>%
  transmute(
    tdomain, tphylum, tclass, torder, tfamily, tgenus, tspecies, 
    est_genome_size = round(est_genome_size)
  ) %>%
  group_by(tdomain, tphylum, tclass, torder, tfamily) %>% 
  summarise(
    n_genomes = n(), 
    mean_genome_size = round(mean(est_genome_size) / 1e6, digits = 1), 
    .groups = 'drop'
  ) %>%
  datatable(
    colnames = c(
      'Domain' = 'tdomain', 'Phylum' = 'tphylum', 'Class' = 'tclass', 'Order' = 'torder',
      'Family' = 'tfamily', 
      'Number of species' = 'n_genomes', 'Mean genome size (Mbp)' = 'mean_genome_size'
    )
  )
```
...
rnr_class_combs %>%
  ...
  inner_join(major_phyla, by = c('tdomain', 'tphylum')) %>%
  group_by(rnr_combination, tdomain, major_phylum, n_genomes) %>%
  summarise(n_combs = sum(n_combs), .groups = 'drop') %>%
