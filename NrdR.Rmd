...
nrdrs %>%
  ...
  inner_join(taxa %>% group_by(tdomain) %>% summarise(genome_count = n(), .groups = 'drop'), by = 'tdomain') %>%
  mutate(proportion = nrdr_pres_count / genome_count) %>%
  ggplot(aes(x=tdomain, y=genome_count, fill=nrdr_pres_count)) +
