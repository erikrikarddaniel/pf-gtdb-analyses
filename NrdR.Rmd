...
(`r figr('num-single-class-rnr-alphas', T, type = 'Figure')`).

```{r num-single-class-rnr-alphas, fig.height = 8, fig.width = 12, fig.cap = '**Number of species in which NrdR is associated with one or more RNRs of the same class.** (*Note:* I know the scale for number of bacterial genomes is strange, but not why.)'}
nrdrs %>% select(genome_accno, tdomain:tspecies) %>% distinct() %>%
  inner_join(
  ...
  mutate(n_rnr_alphas = ifelse(n_rnr_alphas > 3, '>3 RNR alphas', sprintf('%d RNR alphas', n_rnr_alphas))) %>%
  inner_join(topphyla %>% select(tdomain, tphylum, top20phyla), by = c('tdomain', 'tphylum')) %>%
  ...
  geom_col() +
  scale_fill_discrete('N. RNR alpha paralogs') +
